...
import com.example.videocropandtrim.utils.widget.TimeLineTrimmerVersion01.TimeLineRectMovingDirectionVersion01.Companion.getTimeLineRectMovingDirectionVersion01ByValue
 ...
 */
class TimeLineTrimmerVersion01 @JvmOverloads constructor(
    context: Context,
    ...
    private var timeLineRectMovingDirection: TimeLineRectMovingDirectionVersion01 = TimeLineRectMovingDirectionVersion01.HORIZONTAL
...
//            val typeArray = context.obtainStyledAttributes(lAttrs, R.styleable.TimeLineTrimmer)
            ...
            lineStrokeColor = typedArray.getColor(
                R.styleable.TimeLineTrimmerVersion01_tlt_ver01_line_stroke_color,
                ContextCompat.getColor(context, R.color.color_default_crop_frame)
            )
            mDimmedColor = typedArray.getColor(
                R.styleable.TimeLineTrimmerVersion01_tlt_ver01_dimmed_color,
                ContextCompat.getColor(context, R.color.color_default_dimmed)
            )

            leftThumb = typedArray.getDrawable(
                R.styleable.TimeLineTrimmerVersion01_tlt_ver01_left_guide_line_thumb
            )

            rightTumb = typedArray.getDrawable(
                R.styleable.TimeLineTrimmerVersion01_tlt_ver01_right_guide_line_thumb
            )
            ...
            timeLineRectMovingDirection = getTimeLineRectMovingDirectionVersion01ByValue(
                typedArray.getInt(
                    R.styleable.TimeLineTrimmerVersion01_tlt_ver01_timeline_rect_direction,
                    TimeLineRectMovingDirectionVersion01.HORIZONTAL.value
                )
            )
...
//        }
    ...
    private fun initFrame(typeArray: TypedArray) {
        val strokeWidth = typeArray.getDimensionPixelSize(
            R.styleable.TimeLineTrimmerVersion01_tlt_ver01_line_stroke_size,
        ...
        when(mTimeLineTrimmerTouchArea){
            ...
            TimeLineTrimmerTouchArea.IN -> {
                ...
                when (timeLineRectMovingDirection) {
                    TimeLineRectMovingDirectionVersion01.ALL -> mTempTimeLineRect.offset(
                        x - mPreviousTouchX,
                        y - mPreviousTouchY
                    )
                    TimeLineRectMovingDirectionVersion01.HORIZONTAL -> mTempTimeLineRect.offset(
                        x - mPreviousTouchX,
                        0f
                    )
                    TimeLineRectMovingDirectionVersion01.VERTICAL -> mTempTimeLineRect.offset(
                        0f,
                        y - mPreviousTouchY
                    )
                }
...
//        }
    ...
    enum class TimeLineRectMovingDirectionVersion01(val value: Int){
        ALL(0),
        VERTICAL(1),
        HORIZONTAL(2);

        companion object {
            fun getTimeLineRectMovingDirectionVersion01ByValue(value: Int = HORIZONTAL.value): TimeLineRectMovingDirectionVersion01 {
